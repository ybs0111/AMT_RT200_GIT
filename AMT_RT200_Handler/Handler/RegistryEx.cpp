...
BOOL CRegistryEx::Write (LPCTSTR pszKey, LPCTSTR pszData)
{
	...
	//char chTemp[255] = "";
	int nwCharLen = strTemp.GetLength()*2;
	ASSERT(m_hKey);
	ASSERT(pszKey);
	ASSERT(pszData);
	ASSERT(AfxIsValidAddress(pszData, nwCharLen, FALSE));
	
	LONG ReturnValue = RegSetValueEx (m_hKey, pszKey, 0L, REG_SZ,
		(CONST BYTE*) pszData, nwCharLen+1);

	m_Info.lMessage = ReturnValue;
	m_Info.dwSize = nwCharLen+1;
...
BOOL CRegistryEx::Read(LPCTSTR pszKey, int& iVal)
{
	...
	LONG lReturn = RegQueryValueEx (m_hKey, (LPCWSTR) pszKey, NULL,
...
//	MessageBox( NULL, (LPCWSTR)msg, (LPCWSTR)lpMsgBuf, MB_OK | MB_ICONINFORMATION );
//	LocalFree( lpMsgBuf ); 
*/
}
