...
//
DWORD CButtonST::SetIcon(UINT nID, int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(UINT nID, int nIconIn, int nIconOut)
{
	return SetIcon(nID, nIconIn, 0, 0, nIconOut, 0, 0);
} // End of SetIcon
...
//
DWORD CButtonST::SetBitmaps(UINT nID, int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
DWORD CButtonST::SetURL(LPCTSTR lpszURL)
{
	...
	if (lpszURL)
	{
		// Store the URL
// jtkim 20130704
		_tcscpy_s(m_szURL, sizeof(m_szURL), lpszURL);
...
//			_tcscpy(m_csSounds[byIndex].szSound, lpszSound);
			_tcscpy_s(m_csSounds[byIndex].szSound, sizeof(m_csSounds[byIndex].szSound), lpszSound);
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	else if (m_bcSet == 1)		//다운 시켰을때만, 색상 변경. ViboX
	{
		if (m_bMouseOnButton)
		{
			if (m_bIsPressed)
			{
				crColor = RGB(201, 201, 151);
			}
			else
				crColor = m_crColors[BTNST_COLOR_BK_IN];
		}
	...
	else if (m_bcSet == 2)		//다운되거나 말거나, XPGroupBox색상과 동일. ViboX
	{
		crColor = RGB(255, 255, 204);
	}
	else if (m_bcSet == 3)		//배경이 Message Box색상과 동일. ViboX
	{
		if (m_bMouseOnButton)
		{
			if (m_bIsPressed)
		...
		{
			if (m_bIsFocused)
				crColor = RGB(206, 206, 206);
			else
				crColor = RGB(206, 206, 206);
		}
	}
	else if (m_bcSet == 4)		//다운되거나 말거나, Message Box색상과 동일. ViboX
	{
		crColor = RGB(206, 206, 206);
	}
...
void CButtonST::SetButtonColor(int n_set)
{
	m_bcSet = n_set;
}
...
void CButtonST::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	GetParent()->PostMessage(WMU_CHILD_LBUTTON_DOWN, m_pID, (LPARAM)this);
...
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	GetParent()->PostMessage(WMU_CHILD_LBUTTON_UP, m_pID, (LPARAM)this);
