...
void CClientSocket::OnReceive(int nErrorCode) 
{
	...
	st_client_info[m_nAddr].strRev = strMsg.Mid(0, m_nLength);

	if (m_pWnd != NULL)
	{
		::SendMessage(m_pWnd->m_hWnd, WM_CLIENT_MSG + m_nAddr, CLIENT_REV, 0);
	}
...
void CClientSocket::OnClose(int nErrorCode) 
{
	if (m_pWnd != NULL)
	{
		::SendMessage(m_pWnd->m_hWnd, WM_CLIENT_MSG + m_nAddr, CLIENT_CLOSE, 0);
	}
...
void CClientSocket::OnConnect(int nErrorCode)
{
	...
	if (m_pWnd != NULL)
	{
		::SendMessage(m_pWnd->m_hWnd, WM_CLIENT_MSG + m_nAddr, CLIENT_ACCEPT, nErrorCode);
	}
