...
void CXLAutomation::ReleaseVariant(VARIANTARG *pvarg)
{
	...
	m_vt = pvarg->vt & 0xfff;		// mask off flags
	...
	if (V_ISARRAY(pvarg)) 
	{
		...
		if (m_vt == VT_VARIANT) 
		{
			...
			if (lUBound > lLBound) 
			{
				...
				SafeArrayAccessData(pvarg->parray, (void**)&m_pvargArray);
				
				for (l = 0; l < lUBound; l++) 
				{
					ReleaseVariant(m_pvargArray);
					m_pvargArray++;
				}
...
//		ReleaseVariant(&m_pdispExcelApp->); 
	}
...
BOOL CXLAutomation::SaveAs(CString szFileName, int nFileFormat, CString szPassword, CString szWritePassword, BOOL bReadOnly, BOOL bBackUp)
{
	...
	ClearAllArgs();
	AddArgumentBool(NULL, 0, FALSE);

	if (!ExlInvoke(m_pdispWorkbook, L"DisplayAlerts", NULL, DISPATCH_METHOD, DISP_FREEARGS))
	{
		return FALSE;
	}
