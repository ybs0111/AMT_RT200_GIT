...
//		wcscpy( lf.lfFaceName, _T("MS Serif") );
		MultiByteToWideChar(CP_ACP, 0, (LPCSTR)(LPCTSTR)_T("MS Serif"), -1, lf.lfFaceName, 32); 
...
BOOL CThumbnailButton::PreTranslateMessage(MSG* pMsg) 
{
	  ...
	  switch( pMsg->message )
	  {
			case WM_RBUTTONDOWN:
				...
				if(st_handler_info.cWndAlarm->m_hWnd != NULL)
				{
					SendMessageTimeout(st_handler_info.cWndAlarm->m_hWnd, UWM_ON_TNB_RCLICKED, (WPARAM)this, (LPARAM)0, SMTO_ABORTIFHUNG|SMTO_NORMAL, 2000, dwResult );
				}
				break;
			...
			case WM_LBUTTONDOWN:
				...
				if(st_handler_info.cWndAlarm->m_hWnd != NULL)
				{
					SendMessageTimeout(st_handler_info.cWndAlarm->m_hWnd, UWM_ON_TNB_LCLICKED, (WPARAM)this, (LPARAM)L_BUTTON_DOWN, SMTO_ABORTIFHUNG|SMTO_NORMAL, 2000, dwResult );
				}
				break;
			...
			case WM_MOUSELEAVE:
				if( !m_bMouseClicked )
				{
					...
					if(st_handler_info.cWndAlarm->m_hWnd != NULL)
					{
						SendMessageTimeout(st_handler_info.cWndAlarm->m_hWnd, UWM_ON_TNB_LCLICKED, (WPARAM)this, (LPARAM)L_BUTTON_UP, SMTO_ABORTIFHUNG|SMTO_NORMAL, 2000, dwResult );
					}
				}
