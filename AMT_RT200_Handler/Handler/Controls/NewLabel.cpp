...
void CTextRotator::Clear()
 {
	::SelectObject(m_hRotatedMemDC, m_hOldRotatedBitmap);
	DeleteObject(m_hRotatedBitmap); // Delete temporary empty bitmap 2
...
BOOL CTextRotator::RotateText(UINT uiAngle)
 {
	...
	//strcpy_s(pszText, sizeof(m_strText), m_strText);
	//strcpy(pszText, m_strText);
	printf(pszText, m_strText);

	char *context = NULL;
	char *psz = strtok_s(pszText, "\n", &context);

	while (psz)
	{
		arrayText.Add((LPCTSTR)psz);
		iLineCounter++;
		psz = strtok_s(NULL, "\n", &context);
	 }
	...
	if (!hEmptyBitmap1) return FALSE;
	 ...
	 m_hRotatedBitmap = CreateCompatibleBitmap(m_hSourceDC, iMax, iMax);
	...
	HBITMAP hOldEmptyBitmap1 = (HBITMAP)::SelectObject(hMemDC, hEmptyBitmap1); // Put the empty bitmap in memory DC

	//::SelectObject(m_hRotatedMemDC, hEmptyBitmap2); // Put the empty bitmap in rotated memory DC
	m_hOldRotatedBitmap = (HBITMAP)::SelectObject(m_hRotatedMemDC, m_hRotatedBitmap); // Put the empty bitmap in rotated memory DC

	//::SelectObject(hMemDC, GetCurrentObject(m_hSourceDC,OBJ_FONT)); // Select same font as source DC 

	// 새롭게 추가함
	HFONT hOldFont = (HFONT)::SelectObject(hMemDC, GetCurrentObject(m_hSourceDC,OBJ_FONT)); // Select same font as source DC 
	/* ****************************************************************************** */
	...
	::SelectObject(hMemDC, hOldEmptyBitmap1);
  ...
  if (bSingleLine || (!bSingleLine && bIsVTop))
   {
    ...
    if (bIsVBottom)
     {
      CSize cSize = pDC->GetTextExtent((CString)lpszText);

      rect.top += (stRect.bottom - stRect.top) - cSize.cy;
     }

    pDC->DrawText((LPCTSTR)lpszText, -1, &rect, uiFormat);

    return;
   }

  // MultiLine Drawing (TOP or BOTTOM)
  pDC->DrawText((LPCTSTR)"**********", -1, &stRect, uiFormat);
 }
...
CString CNewLabel::MakeVerticalText(LPCSTR lpszText)
 {
  ...
  strSource = (lpszText) ? (LPCTSTR)lpszText : m_strText;
...
CNewLabel &CNewLabel::SetFontName(LPCTSTR szFaceName)
 {
  //strcpy_s(m_stLF.lfFaceName, sizeof(szFaceName), szFaceName);
  //strcpy(m_stLF.lfFaceName, szFaceName);
  //printf(m_stLF.lfFaceName, szFaceName);
	_tcscpy_s(m_stLF.lfFaceName, 100, szFaceName) ;
...
CNewLabel &CNewLabel::SetText(LPCTSTR szText)
 {
  ...
  if (m_bVerticalText)
   {
	    SetWindowText( MakeVerticalText((LPCSTR)szText) );
   }
...
CNewLabel &CNewLabel::SetToolTipText(LPCTSTR lpszText)
 {
	  m_cControlToolTip.SetText((LPCSTR)lpszText);
	 return *this;
 }
...
CNewLabel &CNewLabel::SetVerticalText(BOOL bSet)
 {
  if (bSet && !m_bVerticalText)
   {
		SetWindowText( MakeVerticalText((LPCSTR)((LPCTSTR)m_strText)));
   }
...
void CNewLabel::OnPaint() 
 {
  ...
  CRect    rectClient;
  CString  strText = (!m_bVerticalText) ? m_strText : MakeVerticalText((LPCSTR)((LPCTSTR)m_strText));
  ...
  if (m_hBackImage &&
      m_uiAngle == 0 &&
      (!IsMultiLineString((LPCSTR)((LPCTSTR)strText)) || (IsMultiLineString((LPCSTR)((LPCTSTR)strText)) && GetVertAlignment() == VERT_TOP)))
   {
    dc.SetTextColor(!m_bUseDefaultTextColor ? m_clrTextColor : ::GetSysColor(COLOR_WINDOWTEXT));

    DrawAlignedText(&dc, (LPCSTR)((LPCTSTR)strText), rectClient, m_uiAlignment);
   }
  else {
