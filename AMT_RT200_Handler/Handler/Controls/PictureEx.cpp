...
BOOL CPictureEx::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
	...
	HRSRC hPicture = FindResource(AfxGetResourceHandle(),(char*)szResourceName,szResourceType);
	HGLOBAL hResData;
	hResData = LoadResource(AfxGetResourceHandle(),hPicture);
	if (!hPicture || !hResData)
	{
...
HGLOBAL CPictureEx::GetNextGraphicBlock(UINT *pBlockLen, 
	int *pDelay, SIZE *pBlockSize, SIZE *pBlockOffset, 
	UINT *pDisposal)
{
