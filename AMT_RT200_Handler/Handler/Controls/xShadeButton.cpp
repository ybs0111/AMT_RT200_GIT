...
void CxShadeButton::SetIcon(UINT nID, UINT nIcon, UINT nIconAlign, UINT nIconDown)
{
	...
	//m_Icon = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIcon), IMAGE_ICON, 0, 0, 0);
	m_ID = nID;
...
//	GetParent()->SendMessage(WM_NOTIFY, BUTTON_MOUSE_DN, (LPARAM)&hdr);
	...
	GetParent()->PostMessage(WMU_CHILD_LBUTTON_DOWN, GetDlgCtrlID(), (LPARAM)this);
...
//	GetParent()->SendMessage(WM_NOTIFY, GetDlgCtrlID(), (LPARAM)&hdr);
	...
	GetParent()->PostMessage(WMU_CHILD_LBUTTON_UP, GetDlgCtrlID(), (LPARAM)this);
    //Default-process the message
    m_button_down = false;
