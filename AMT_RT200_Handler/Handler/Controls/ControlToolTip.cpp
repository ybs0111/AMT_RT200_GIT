...
CSize CControlToolTip::GetMultiLineTextExtent(CClientDC *pDC, LPCTSTR lpszText)
 {
  ...
  //strcpy_s(sz, sizeof(lpszText), lpszText);
  //strcpy(sz, lpszText);
  printf(sz, lpszText);

   char *context = NULL;
  char *psz = strtok_s(sz,"\n", &context);

	while (psz != NULL)
	{
		size = pDC->GetTextExtent((CString)psz);
...
void CControlToolTip::RegisterWindowClass()
 {
  ...
  if ( !(::GetClassInfo(hInst, (LPCWSTR)"ControlToolTip",&stWndClass)) )
   {
    ...
    stWndClass.lpszClassName = (LPCWSTR)CONTROLTOOLTIP_CLASSNAME;
...
BOOL CControlToolTip::Attach(CWnd *pControl)
 {
  ...
  BOOL bRet = CreateEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST,
                       ...
                       NULL);
/*
  BOOL bRet = CreateEx(WS_EX_TOOLWINDOW,
                       (LPWSTR)"ControlToolTip",
                       NULL,
                       WS_BORDER | WS_POPUP,
                       0,
...
*/
  ...
  SetText((LPCSTR)((LPCTSTR)m_strText));
