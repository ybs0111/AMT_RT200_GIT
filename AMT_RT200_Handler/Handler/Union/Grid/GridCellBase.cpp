...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	//irehyun 2009.07.17 grid의 선이 사라지는 것을 방지하기 위해 grid cell이 fixed가 아니면 약간 작게 출력한다.
	if (pGrid->IsCellFixed(nRow, nCol) == false) {
		rect.left += 1;
		rect.top += 1;
		rect.bottom -= 1;
		rect.right -=1;
    ...
    if (pGrid->GetImageList() && GetImage() >= 0)
    {
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            //  working with CMEMGridDC directly.  Instead, don't display image
...
//       rect.SetRect(0,0, nMaxWidth+1, 0);
		// jtkim 20130523 
		rect.SetRect(0,0, 1000, 10);
 //       pDC->DrawText(szText, -1, rect, nFormat | DT_CALCRECT);
		pDC->DrawText(szText, -1, rect, nFormat | DT_NOPREFIX);
        size = rect.Size();
    }
    else
        size = pDC->GetTextExtent(szText, sizeof(szText)/*_tcslen(szText)*/);
