...
void CGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	// 20150113 jtkim
	SendMessageToParent(-1, -1, NM_CLICK);
}
            ...
            if (nFixedRows > m_nFixedRows)
            {
                for (int i = m_nFixedRows; i < nFixedRows; i++)
                    for (int j = 0; j < GetColumnCount(); j++)
                    {
                        ...
                        SetItemBkColour(i, j, CLR_DEFAULT, CLR_DEFAULT );
...
BOOL CGridCtrl::SetItemBkColour(int nRow, int nCol, COLORREF bkcr, COLORREF textcr)
{
    if (GetVirtualMode())
        return FALSE;
    ...
    pCell->SetBackClr(bkcr);
	pCell->SetTextClr(textcr);
    return TRUE;
}
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
             ...
             m_MouseMode != MOUSE_OVER_ROW_DIVIDE)        
    {
			...
			m_LeftClickDownCell.col >= GetFixedColumnCount())
		{
			...
			//SetFocusCell(-1, -1);
			SetFocusCell(m_LeftClickDownCell.row, -1); //irehyun 2000.07.08 수정 row선택시에 선택 줄 event를 받기 위함
    ...
    TRY
    {
        ...
        *end = _T('\0');
        ...
        while (token)
        {
			// jtkim 20130523 수정
			//InsertColumn(token)
            InsertColumn(token, DT_WORDBREAK | DT_NOPREFIX | DT_LEFT);
